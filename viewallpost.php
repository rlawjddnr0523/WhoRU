...
<div class="wrapper d-flex align-items-stretch">
    ...
    <div id="content" class="p-4 p-md-5">
        ...
        <h6 class="mb-4">지금까지 올라온 모든 게시물..</h6>
        <?php
        $conn = mysqli_connect("localhost", "kjwook0523", "Kimmark4167*", "kjwook0523");
        $query = "SELECT * FROM blog";
        $result = mysqli_query($conn, $query);
        while($row = mysqli_fetch_array($result)){
            ?><a href="blogview.php?id=<?php echo "$row[contents_id]"; ?>" class="mb-4"><h5><?php echo "$row[blog_title]";?></h5></a><h6><?php echo "$row[writed]"; ?></h6><br/><?php
        }
        ?>
